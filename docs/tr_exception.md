...
### 6. ShapeError（V1.42.1新增）
形状错误异常，用于处理张量形状不匹配或无效形状的错误。

**使用场景**:
```cpp
if (tensor_a.numel() != tensor_b.numel()) {
    TR_THROW_SHAPE_ERROR("Shape mismatch: tensor A has " +
                         std::to_string(tensor_a.numel()) + " elements, " +
                         "but tensor B has " + std::to_string(tensor_b.numel()) + " elements");
}

if (target_shape.n() == 0 || target_shape.c() == 0 || target_shape.h() == 0 || target_shape.w() == 0) {
    TR_THROW_SHAPE_ERROR("Target shape cannot contain zero dimensions: " + target_shape.to_string());
}
```
...
```cpp
// reshape操作中的形状验证
Tensor reshape(const Tensor& input, const Shape& target_shape) {
    if (input.numel() != target_shape.numel()) {
        throw ShapeError("Cannot reshape: element count mismatch between input " +
                         input.shape().to_string() + " (" + std::to_string(input.numel()) +
                         " elements) and target " + target_shape.to_string() +
                         " (" + std::to_string(target_shape.numel()) + " elements)");
    }
    // 继续处理...
}
```
...
TR_THROW_SHAPE_ERROR("Shape mismatch");        // ShapeError
```
...
- **ShapeError**: 张量形状不匹配、无效形状验证（V1.42.1新增）
...
### V1.42.1更新内容（2025-11-15）
- **新增ShapeError异常**：专门用于处理张量形状不匹配和无效形状验证
- **新增TR_THROW_SHAPE_ERROR宏**：便捷的形状错误抛出宏
- **完善异常分类**：将异常类型从6种扩展到7种，提升错误处理的精确性
- **应用场景扩展**：支持reshape、维度操作等形状相关功能的错误处理

### 版本历史
- **V1.10.00**: 初始版本，建立6种异常类型的基础架构
- **V1.42.1**: 新增ShapeError异常，完善形状验证错误处理
