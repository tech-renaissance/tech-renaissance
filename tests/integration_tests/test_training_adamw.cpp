/**
 * @file test_training_adamw.cpp
 * @brief AdamW训练对齐测试 - 与PyTorch完全对齐
 * @details 使用TSR文件验证我们的AdamW优化器与PyTorch的训练一致性
 * @version 1.54.0
 * @date 2025-11-19
 * @author 技术觉醒团队
 */
...
int main() {
    std::cout << "=== PyTorch Training Alignment Test (AdamW) ===" << std::endl;
    std::cout << "Testing 2-layer NN (4-5-2) with AdamW optimizer" << std::endl;
    std::cout << "===============================================" << std::endl;
    ...
    try {
        ...
        // 4. 创建AdamW优化器
        std::cout << "\n4. Creating AdamW optimizer..." << std::endl;
        ...
        if (passed_tests == total_tests) {
            std::cout << "\n[SUCCESS] All tests PASSED! Our AdamW optimizer is perfectly aligned with PyTorch!" << std::endl;
            return 0;
        } else {
            std::cout << "\n[FAILURE] Some tests FAILED! There are alignment issues with PyTorch AdamW." << std::endl;
            return 1;
        }
