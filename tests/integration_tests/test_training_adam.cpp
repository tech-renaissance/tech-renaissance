/**
 * @file test_training_adam.cpp
 * @brief Adam训练对齐测试 - 与PyTorch完全对齐
 * @details 使用TSR文件验证我们的Adam优化器与PyTorch的训练一致性
 * @version 1.53.0
 * @date 2025-11-19
 * @author 技术觉醒团队
 */
...
int main() {
    ...
    std::cout << "Testing 2-layer NN (4-5-2) with Adam optimizer" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        std::cout << "\n4. Creating Adam optimizer..." << std::endl;
        auto optimizer = std::make_unique<Adam>(0.001f, 0.9f, 0.999f, 1e-8f, 1e-4f, backend);
