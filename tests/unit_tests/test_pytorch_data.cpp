 ...
 * @brief 进一步测试PythonSession类
 * @details 借助test_python_session.cpp的成功经验，进一步测试用PyTorch生成数据
 * @version 1.19.01
 * @date 2025-10-29
 * @author 技术觉醒团队
 * @note 依赖项: PythonSession, Tensor
 * @note 所属系列: tests
 */
...
bool test_python_natural_exit() {
    ...
    try {
        PythonSession session(PYTHON_SCRIPT_PATH, "natural_exit_test");
        session.start();
...
bool test_manual_termination() {
    ...
    try {
        PythonSession session(PYTHON_SCRIPT_PATH, "manual_terminate_test");
        session.start();
...
bool test_matmul_style_a() {
    ...
    try {
        ...
        PythonSession session(PYTHON_SCRIPT_PATH, "matmul_test_a");
        session.start();
...
bool test_add_style_b() {
    ...
    try {
        ...
        PythonSession session(PYTHON_SCRIPT_PATH, "add_test_b");
        session.start();
