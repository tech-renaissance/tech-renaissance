...
struct TestCase {
    ...
    bool needs_pytorch_validation;  // 是否需要PyTorch验证
};
...
int main() {
    ...
    std::vector<TestCase> test_cases = {
        ...
        {
            "reshape", "reshape",
            [](CpuBackend& backend, const Tensor& input) {
                // reshape测试：将2x3x4x5的张量reshaped为1x6x20x1
                return backend.reshape(input, Shape(1, 6, 20, 1));
            },
            [](CpuBackend& backend, Tensor& input) {
                // inplace reshape测试：将2x3x4x5的张量reshaped为1x6x20x1
                backend.reshape_inplace(input, Shape(1, 6, 20, 1));
            },
            [](CpuBackend& backend, const Tensor& input, Tensor& output) {
                // into reshape测试：将2x3x4x5的张量reshaped为1x6x20x1
                backend.reshape_into(input, output, Shape(1, 6, 20, 1));
            },
            false  // 不需要PyTorch验证
        },
        {
            "tanh", "tanh",
            [](CpuBackend& backend, const Tensor& input) { return backend.tanh(input); },
            [](CpuBackend& backend, Tensor& input) { backend.tanh_inplace(input); },
            [](CpuBackend& backend, const Tensor& input, Tensor& output) { backend.tanh_into(input, output); },
            false  // 不需要PyTorch验证
        },
        {
            "dtanh", "dtanh",
            [](CpuBackend& backend, const Tensor& input) { return backend.dtanh(input); },
    ...
    for (size_t i = 0; i < test_cases.size(); ++i) {
        ...
        try {
            ...
            if (test_case.needs_pytorch_validation) {
                // PyTorch验算
                ...
                // 不需要PyTorch验证，只打印结果
                std::cout << "  Non-inplace: SUCCESS" << std::endl;
                std::cout << "    Input shape: " << input.shape().to_string() << ", Output shape: " << cpu_result.shape().to_string() << std::endl;

                // 只显示前几个元素的值
                if (cpu_result.numel() > 0) {
                    const float* data = static_cast<const float*>(cpu_result.data_ptr());
                    std::cout << "    Sample values: [" << data[0];
                    if (cpu_result.numel() > 1) std::cout << ", " << data[1];
                    if (cpu_result.numel() > 2) std::cout << ", " << data[2];
                    std::cout << ", ...]" << std::endl;
                }
                passed++;
            }
        ...
        try {
            ...
            if (test_case.needs_pytorch_validation) {
                ...
                // 不需要PyTorch验证，只打印结果
                std::cout << "  Inplace:     SUCCESS" << std::endl;
                std::cout << "    Original shape: " << original_shape.to_string() << ", Result shape: " << input.shape().to_string() << std::endl;

                // 只显示前几个元素的值
                if (input.numel() > 0) {
                    const float* data = static_cast<const float*>(input.data_ptr());
                    std::cout << "    Sample values: [" << data[0];
                    if (input.numel() > 1) std::cout << ", " << data[1];
                    if (input.numel() > 2) std::cout << ", " << data[2];
                    std::cout << ", ...]" << std::endl;
                }
                passed++;
            }
        ...
        try {
            ...
            // 创建输出张量（transpose和reshape需要特殊处理输出形状）
            Tensor output;
            if (test_case.name == "transpose") {
                // 转置的输出形状是输入形状的交换
                output = cpu_backend->uniform(Shape(3, 2), -100.0f, 100.0f, static_cast<int>(time(nullptr)) + i + 3000);
            } else if (test_case.name == "reshape") {
                // reshape的输出形状是1x6x20x1
                output = cpu_backend->uniform(Shape(1, 6, 20, 1), -100.0f, 100.0f, static_cast<int>(time(nullptr)) + i + 3000);
            } else {
                // 创建随机同形的输出张量来测试覆盖能力
            ...
            if (test_case.needs_pytorch_validation) {
                ...
                // 不需要PyTorch验证，只打印结果
                std::cout << "  Into:        SUCCESS" << std::endl;
                std::cout << "    Input shape: " << input.shape().to_string() << ", Output shape: " << output.shape().to_string() << std::endl;

                // 只显示前几个元素的值
                if (output.numel() > 0) {
                    const float* data = static_cast<const float*>(output.data_ptr());
                    std::cout << "    Sample values: [" << data[0];
                    if (output.numel() > 1) std::cout << ", " << data[1];
                    if (output.numel() > 2) std::cout << ", " << data[2];
                    std::cout << ", ...]" << std::endl;
                }
                passed++;
            }
