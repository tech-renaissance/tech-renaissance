...
bool test_python_natural_exit() {
    ...
    try {
        PyTorchSession session("default", "natural_exit_test");
        session.start();
...
bool test_manual_termination() {
    ...
    try {
        PyTorchSession session("default", "manual_terminate_test");
        session.start();
        ...
        if (session.is_alive()) {
            ...
            std::cout << "[TEST] Python process terminated manually." << std::endl;
        }
...
bool test_matmul_style_a() {
    ...
    try {
        ...
        PyTorchSession session("default", "matmul_test_a");
        session.start();
...
bool test_add_style_b() {
    ...
    try {
        ...
        PyTorchSession session("default", "add_test_b");
        session.start();
