 ...
 * @file test_print.cpp
 * @brief 张量打印功能测试
 * @details 测试4D张量的打印功能，与PyTorch输出格式对齐，并通过PyTorchSession实时对比
 * @version 1.00.00
 * @date 2025-10-27
 * @author 技术觉醒团队
 * @note 依赖项: Tensor, Backend, PyTorchSession
 * @note 所属系列: tests
 */
...
// 与Python对比的辅助函数
void compare_with_pytorch(const Tensor& tensor, const std::string& name, PyTorchSession& session, int precision = 6) {
    try {
...
bool test_cpu_fp32_print() {
    ...
    try {
        // 启动Python对比会话
        PyTorchSession session(PYTHON_SCRIPT_PATH, "cpu_fp32_compare");
        session.start();
