 ...
 * @date 2025-10-29
 * @author 技术觉醒团队
 * @note 依赖项: tensor.h, cpu_backend.h, python_session.h
 * @note 所属系列: tests
 */
...
bool test_pytorch_comparison() {
    ...
    try {
        ...
        // 使用PythonSession进行对比计算
        std::cout << "\n[TEST] Starting Python session for comparison..." << std::endl;
        PythonSession session(PYTHON_SCRIPT_PATH, "cpu_mm_comparison");
        session.start();
