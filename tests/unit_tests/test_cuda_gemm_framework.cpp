...
int main() {
    ...
    try {
        ...
        // 1. 初始化PyTorch会话
        tr::PyTorchSession pytorch_session(python_script_path, "cuda_gemm_pytorch_test");
        pytorch_session.start();
        ...
        pytorch_session.send_tensor(a_pytorch, "a");
        pytorch_session.send_tensor(b_pytorch, "b");

        // 获取PyTorch计算结果
        tr::Tensor c_pytorch = pytorch_session.fetch_tensor(R"({"cmd": "matmul", "params": "a,b"})", 10000);
        ...
        // 清理PyTorch会话
        pytorch_session.send_request(R"({"cmd": "exit"})");
