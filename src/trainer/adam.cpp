...
void Adam::update_parameter(Tensor& param, const Tensor& grad, OptimizerState& state, size_t param_index) {
    ...
    // 【按照Algorithm.md重新实现Adam算法】

    // Step1: grad = grad + weight_decay * weight (权重衰减在梯度更新前执行)
    ...
    // Step2: m = beta1 * m + (1 - beta1) * grad (使用修改后的梯度)
    // temp2 = (1 - beta1) * temp1
    backend_->mul_into(temp1_buffers_[param_index], 1.0f - beta1_, temp2_buffers_[param_index]);
    // m *= beta1
    backend_->mul_inplace(state.adam_m, beta1_);
    // m = m + temp2
    backend_->add_into(state.adam_m, temp2_buffers_[param_index], state.adam_m);

    // Step3: m_hat = m / (1.0 - pow(beta1, time_step))，同时预乘学习率
    ...
    // Step4: v = beta2 * v + (1 - beta2) * pow(grad, 2)
    // temp1 = square(temp1) (temp1存储修改后的梯度)
    backend_->square_inplace(temp1_buffers_[param_index]);
    // temp1 *= (1.0 - beta2)
    ...
    // Step5: v_hat = v / (1 - pow(beta2, time_step))
    ...
    // Step6: weight = weight - temp2 / (sqrt(temp1) + eps)
    // temp1 = sqrt(temp1)
