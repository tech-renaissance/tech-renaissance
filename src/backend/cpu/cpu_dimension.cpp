...
Tensor CpuBackend::view(const Tensor& input, const Shape& new_shape) {
    // 1. 有效性检查
    validate_same_device(input.device());

    if (input.numel() != new_shape.numel()) {
        throw TRException("[CpuBackend::view] Shape mismatch: cannot view a tensor with " +
    ...
    if (!input.is_contiguous()) {
        throw TRException("[CpuBackend::view] view is only supported for contiguous tensors in the current version.");
    }
